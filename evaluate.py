...
if __name__ == '__main__':
    ...
    delete_path(args.check_dir)
    delete_path(os.path.join(args.data_dir, 'Further_Learned_Features'))

    for item in ratios:
        args.pooling_ratio = item
        ...
        for time in range(10):
            # set random seed
            args.seed = np.random.randint(200)
            torch.manual_seed(args.seed)

            print('Pooling ratio: %.3f, random seed: %d, times: %d' % (args.pooling_ratio, args.seed, time+1))
            ...
            if args.logistic:
                logistic(downsample.shape[0], args, time)
            ...
            if args.gcn:
                ...
                # run GCN
                kfold_gcn(edge_index, edge_attr, downsample.shape[0], time, args)
